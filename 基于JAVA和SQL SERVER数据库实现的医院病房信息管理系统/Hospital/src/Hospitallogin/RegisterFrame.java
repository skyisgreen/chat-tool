		...
		if(source == button1) {
			...
			try {
				if("".equals(no) || "".equals(password) || "".equals(name) || "".equals(age) || "".equals(tel) || "".equals(sex.trim())|| "".equals(title.trim()) || "".equals(deptname.trim())) {
					...
					try {
						if(!rs.next()) {
							int a =JOptionPane.showConfirmDialog(null, "确认注册？","系统提示",JOptionPane.YES_NO_OPTION);
							...
							JOptionPane.showMessageDialog(null, "该用户已经存在，无法注册!","系统提示",JOptionPane.ERROR_MESSAGE);
						}
					} catch (SQLException e1) {
						// TODO Auto-generated catch block
						e1.printStackTrace();
