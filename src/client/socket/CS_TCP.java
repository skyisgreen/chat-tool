...
public class CS_TCP extends TCP {
	...
	public void dealWithMessage(Flag flag, String message) {
		...
		case GET_FRIEND_INFO:
			...
			break;// 如果是服务器发来的指定好友信息
		case FRIENDS_LIST:
			doFriendList(message);
			break;// 接收服务器发来的刷新好友列表信息
		case QUN_CHAT:
			doQunChat(message);
			break;// 处理群消息
		case PUBLIC_MESSAGE:
			doPublicMessage(message);
			break;// 处理公告信息
		case SHOW_WINDOW:
			doShowWindow(message);
			break;// 处理公告信息
	...
	private void doShowWindow(String message) {
	...
	public void doLogin(String message) {
		...
		{
			JOptionPane.showMessageDialog(null, message.split(MyTools.SPLIT1)[1]);// 显示登录失败的原因
		}
	}
